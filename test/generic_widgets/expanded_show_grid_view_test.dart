...
void main() {
  ...
  testWidgets('expanded show grid view should render properly',
      (WidgetTester tester) async {
    List<Show> showList = <Show>[
      Show(
          id: 1,
          name: 'True Detective',
          image: null,
    ...
    await tester.pumpWidget(widgetSetupToBeTested(
        child: ExpandedShowsGridView(
      showList: showList,
    )));
    await tester.pump();

    final Finder gridView =
        find.byKey(const Key(Constants.gridViewExpandedKey));

    expect(gridView, findsOneWidget);
  });

  testWidgets(
      'expanded show grid view should show footer with an opacity of 0.6',
      (WidgetTester tester) async {
    List<Show> showList = <Show>[
      Show(
          id: 1,
    ...
    final Card card = find
        .byKey(const Key(Constants.gridViewCardKey))
        .evaluate()
        .single
        .widget as Card;
    ...
    final GridTile cardTile = find
        .byKey(const Key(Constants.gridViewTileKey))
        .evaluate()
        .single
        .widget as GridTile;
